...
export async function getServiceChat(ticketId: string) {
  const { data } = await axios.get(`tickets/${ticketId}`);
  return data;
}
...
export const createBookingProposal = async (payload: {
  ...
  const { data } = await axios.post("/customer/booking/upsert", payload);
  return data;
};

export const verifyBookingStripeSession = async (sessionId: string) => {
  const res = await axios.get(`/customer/booking/payment/verify?session_id=${sessionId}`);
  return res.data;
};