...
export default function TutorialForm({ onClose, tutorial }: Props) {
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(tutorial?.image_url || null);
  ...
  const renderVideoPreview = () => {
    ...
    // YOUTUBE
    if (videoPreview.includes('youtube.com') || videoPreview.includes('youtu.be')) {
      const match =
    ...
    if (videoPreview.includes('vimeo.com')) {
      const match = videoPreview.match(/vimeo\.com\/(\d+)/);
      const videoId = match ? match[1] : null;
      if (!videoId) return <p className="text-sm text-red-500">Invalid Vimeo URL</p>;
    ...
    return <p className="text-sm text-gray-500 italic">Preview not available for this URL type.</p>;
  };
